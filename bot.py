...
KEYS_API_URL     = os.getenv("KEYS_API_URL", "")  # URL to auth bot's /keys endpoint (optional)
...
async def activate(interaction: discord.Interaction, key: str):
    ...
    if not result["valid"]:
        await interaction.followup.send(
            embed=emb("❌ Invalid Key", f"Validation failed: `{result['message']}`\nContact the bot owner for a valid key.", color=0xED4245),
            ephemeral=True
        )
        return
    ...
    await interaction.followup.send(
        embed=emb(
            "✅ Bot Activated!",
            f"Server successfully activated!\n\n"
            f"**Duration:** {duration}\n"
...
class TicketFormModal(discord.ui.Modal, title="🎫 Create a Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=emb("✅ Ticket Created", f"Your ticket: {channel.mention}", color=0x57F287),
            ephemeral=True
        )
...
class TicketActionsView(discord.ui.View):
    ...
    async def claim(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not has_any_role(interaction.user, [int(r) for r in cfg.get("ticket_support_roles", [])]):
            await interaction.response.send_message(embed=emb("❌ No Permission", "Only support roles can claim.", color=0xED4245), ephemeral=True)
            return
        tickets = get_tickets()
        tid = str(interaction.channel.id)
        if tid not in tickets:
            await interaction.response.send_message(embed=emb("❌ Error", "Ticket data not found.", color=0xED4245), ephemeral=True)
            return
        if tickets[tid]["claimed_by"]:
            claimer = interaction.guild.get_member(int(tickets[tid]["claimed_by"]))
            await interaction.response.send_message(embed=emb("⚠️ Already Claimed", f"By {claimer.mention if claimer else 'someone'}.", color=0xFEE75C), ephemeral=True)
            return
    ...
    async def close_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = get_config()
        if not has_any_role(interaction.user, [int(r) for r in cfg.get("ticket_support_roles", [])]):
            await interaction.response.send_message(embed=emb("❌ No Permission", "Only support roles can close.", color=0xED4245), ephemeral=True)
            return
...
class ConfirmTradeView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        tid = str(interaction.channel.id)
        if tid not in tickets:
            await interaction.response.send_message(embed=emb("❌ Error", "Ticket not found.", color=0xED4245), ephemeral=True)
            return
        uid = str(interaction.user.id)
        if uid in tickets[tid].get("confirm_users", []):
            await interaction.response.send_message(embed=emb("⚠️ Already Confirmed", "You already confirmed.", color=0xFEE75C), ephemeral=True)
            return
...
async def setticketcategory(ctx, category_id: str):
    ...
    if category:
        try:
            ...
            await ctx.send(embed=emb("✅ Updated", f"Set but couldn't hide: `{e}`", color=0xFEE75C))
    else:
        await ctx.send(embed=emb("✅ Updated", "Ticket category set.", color=0x57F287))
...
async def unclaim(ctx):
    ...
    if not tickets[tid]["claimed_by"]:
        await ctx.send(embed=emb("⚠️ Not Claimed", "Not claimed yet.", color=0xFEE75C)); return
    ...
    await ctx.send(
        ...
        embed=discord.Embed(title="🔓 Ticket Unclaimed", description=f"{ctx.author.mention} unclaimed. Needs new support!", color=0xFEE75C)
    )
...
async def adduser(ctx, *, user_input: str):
    ...
    tickets = get_tickets()
    if str(ctx.channel.id) not in tickets:
        await ctx.send(embed=emb("❌ Error", "Use in a ticket channel.", color=0xED4245)); return
    member  = None
    ...
    try:
        ...
        pass
    if not member:
        await ctx.send(embed=emb("❌ Not Found", f"Could not find `{user_input}`.", color=0xED4245)); return
...
async def sendmsg(ctx, channel_id: str = None, *, message: str = None):
    ...
    try:
        ...
        await ctx.send(embed=emb("❌ Error", str(e), color=0xED4245))
...
`!setlogchannel <id>` — Set log channel for transcripts
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        await ctx.send(embed=emb("❌ No Permission", "You need Administrator permission.", color=0xED4245))
        return
    print(f"Error: {error}")
