...
KEYAUTH_APP_NAME = os.getenv("KEYAUTH_APP_NAME", "TicketBot")
...
# ── KeyAuth Verify ────────────────────────────────────────────────────────────
def verify_keyauth_key(key: str) -> dict:
    try:
        init_resp = requests.post(KEYAUTH_API_URL, data={
            "type": "init", "ver": KEYAUTH_VERSION, "hash": "",
            "name": KEYAUTH_APP_NAME, "ownerid": KEYAUTH_OWNER_ID
        }, timeout=10).json()
        if not init_resp.get("success"):
            return {"success": False, "message": init_resp.get("message", "Init failed")}
        sessionid = init_resp["sessionid"]
        lic_resp = requests.post(KEYAUTH_API_URL, data={
            "type": "license", "key": key, "hwid": "SERVER",
            "sessionid": sessionid, "name": KEYAUTH_APP_NAME, "ownerid": KEYAUTH_OWNER_ID
        }, timeout=10).json()
        return {"success": lic_resp.get("success", False), "message": lic_resp.get("message", "Unknown error")}
...
async def close_ticket_channel(channel, closer):
    ...
    if tid in tickets:
        ...
        del tickets[tid]
        save_tickets(tickets)
    await channel.send(embed=discord.Embed(title="🔒 Ticket Closing", description=f"Closed by {closer.mention}. Deletes in 5 seconds.", color=0xED4245))
...
async def activate(interaction: discord.Interaction, key: str):
    ...
    result = verify_keyauth_key(key)
    if not result["success"]:
        await interaction.followup.send(embed=embed("❌ Invalid Key", f"Validation failed: `{result['message']}`", color=0xED4245), ephemeral=True)
        return
    servers = get_servers()
    ...
    await interaction.followup.send(embed=embed("✅ Bot Activated!", f"Server activated successfully!\n\n**Key:** ||{key}||\n**By:** {interaction.user.mention}\n\nRun `!help` to see all commands.", color=0x57F287), ephemeral=True)
...
class TicketOptionButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if not is_server_authorized(interaction.guild.id):
            await send_not_authorized(interaction.channel)
            return
...
class TicketFormModal(discord.ui.Modal, title="🎫 Create a Ticket"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        elif user_not_in_server and raw:
            other_display = f"⚠️ Not found (`{raw}`)"
        elif raw:
            other_display = raw
        else:
            other_display = "N/A"
        ...
        tickets[str(channel.id)] = {
            ...
            "other_user_display": other_display,
            "option": self.option_label,
            "issue": self.trade.value,
        ...
        await interaction.response.send_message(embed=embed("✅ Ticket Created", f"Your ticket: {channel.mention}", color=0x57F287), ephemeral=True)
...
class TicketActionsView(discord.ui.View):
    ...
    async def claim(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not has_any_role(interaction.user, [int(r) for r in cfg.get("ticket_support_roles", [])]):
            await interaction.response.send_message(embed=embed("❌ No Permission", "Only support roles can claim tickets.", color=0xED4245), ephemeral=True)
            return
        tickets = get_tickets()
        ...
        if tickets[tid]["claimed_by"]:
            ...
            await interaction.response.send_message(embed=embed("⚠️ Already Claimed", f"Claimed by {claimer.mention if claimer else 'someone'}.", color=0xFEE75C), ephemeral=True)
            return
        ...
        await interaction.response.send_message(embed=embed("✅ Ticket Claimed", f"{interaction.user.mention} claimed this ticket!", color=0x57F287))
    ...
    async def close_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = get_config()
        if not has_any_role(interaction.user, [int(r) for r in cfg.get("ticket_support_roles", [])]):
            await interaction.response.send_message(embed=embed("❌ No Permission", "Only support roles can close tickets.", color=0xED4245), ephemeral=True)
            return
...
class ConfirmTradeView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if uid in tickets[tid].get("confirm_users", []):
            await interaction.response.send_message(embed=embed("⚠️ Already Confirmed", "You already confirmed.", color=0xFEE75C), ephemeral=True)
            return
...
async def ticketpanel(ctx, image_url: str = None):
    ...
    if not options:
        await ctx.send(embed=embed("❌ No Options", "Add options with `!addticketoption <label> [emoji]`.", color=0xED4245)); return
...
async def setlogchannel(ctx, channel_id: str):
    ...
    if not ch: await ctx.send(embed=embed("❌ Error", "Channel not found.", color=0xED4245)); return
    cfg = get_config()
    cfg["ticket_log_channel"] = channel_id
    save_config(cfg)
    await ctx.send(embed=embed("✅ Log Channel Set", f"Ticket logs → {ch.mention}.", color=0x57F287))
...
async def setsupportrole(ctx, role_id: str):
    ...
    if role_id not in cfg["ticket_support_roles"]:
        ...
        save_config(cfg)
    await ctx.send(embed=embed("✅ Updated", f"Role `{role_id}` is now a support role.", color=0x57F287))
...
async def setfreerole(ctx, role_id: str):
    ...
    if role_id not in cfg["ticket_free_roles"]:
        ...
        save_config(cfg)
    await ctx.send(embed=embed("✅ Updated", f"Role `{role_id}` can msg without claiming.", color=0x57F287))
...
async def setsendrole(ctx, role_id: str):
    if not auth_check(ctx): await send_not_authorized(ctx); return
    cfg = get_config()
    cfg.setdefault("sendmsg_roles", [])
    if role_id not in cfg["sendmsg_roles"]:
        cfg["sendmsg_roles"].append(role_id)
        save_config(cfg)
    await ctx.send(embed=embed("✅ Updated", f"Role `{role_id}` can use `!sendmsg`.", color=0x57F287))
...
async def setticketcategory(ctx, category_id: str):
    ...
    if category:
        try:
            ...
            await ctx.send(embed=embed("✅ Updated", "Ticket category set and hidden.", color=0x57F287))
        except Exception as e:
            await ctx.send(embed=embed("✅ Updated", f"Set but couldn't hide: `{e}`", color=0xFEE75C))
    else:
        await ctx.send(embed=embed("✅ Updated", "Ticket category set.", color=0x57F287))
...
async def setpanelchannel(ctx, channel_id: str):
    ...
    save_config(cfg)
    await ctx.send(embed=embed("✅ Panel Channel Set", f"Panel → {ch.mention}.", color=0x57F287))
...
async def setpanelmsg(ctx, *, content: str = None):
    ...
    if not content or "|" not in content:
        await ctx.send(embed=embed("❌ Usage", "`!setpanelmsg <title> | <description>`", color=0xED4245)); return
    parts = content.split("|", 1)
    cfg = get_config()
    cfg["ticket_panel_title"] = parts[0].strip()
    cfg["ticket_panel_description"] = parts[1].strip()
    save_config(cfg)
    await ctx.send(embed=embed("✅ Updated", "Panel title & description updated.", color=0x57F287))
...
async def addticketoption(ctx, label: str, emoji: str = None):
    ...
    save_config(cfg)
    await ctx.send(embed=embed("✅ Option Added", f"Button `{emoji or ''} {label}` added.", color=0x57F287))
...
async def claim(ctx):
    ...
    if not has_any_role(ctx.author, [int(r) for r in cfg.get("ticket_support_roles", [])]):
        await ctx.send(embed=embed("❌ No Permission", "Only support roles can claim.", color=0xED4245)); return
    tickets = get_tickets()
    tid = str(ctx.channel.id)
    if tid not in tickets:
        await ctx.send(embed=embed("❌ Error", "Not a ticket channel.", color=0xED4245)); return
    if tickets[tid]["claimed_by"]:
        claimer = ctx.guild.get_member(int(tickets[tid]["claimed_by"]))
        await ctx.send(embed=embed("⚠️ Already Claimed", f"By {claimer.mention if claimer else 'someone'}.", color=0xFEE75C)); return
    tickets[tid]["claimed_by"] = str(ctx.author.id)
    tickets[tid]["claimed_by_name"] = str(ctx.author)
    tickets[tid]["claimed_at"] = time.strftime("%Y-%m-%d %H:%M:%S")
    save_tickets(tickets)
    await ctx.channel.set_permissions(ctx.author, read_messages=True, send_messages=True, use_application_commands=True)
    await ctx.send(embed=embed("✅ Claimed", f"{ctx.author.mention} claimed this ticket!", color=0x57F287))
...
async def unclaim(ctx):
    ...
    cfg = get_config()
    if not has_any_role(ctx.author, [int(r) for r in cfg.get("ticket_support_roles", [])]):
        await ctx.send(embed=embed("❌ No Permission", "Only support roles can unclaim.", color=0xED4245)); return
    tickets = get_tickets()
    tid = str(ctx.channel.id)
    if tid not in tickets:
        await ctx.send(embed=embed("❌ Error", "Not a ticket channel.", color=0xED4245)); return
    if not tickets[tid]["claimed_by"]:
        await ctx.send(embed=embed("⚠️ Not Claimed", "Not claimed yet.", color=0xFEE75C)); return
    tickets[tid]["claimed_by"] = None
    tickets[tid]["claimed_by_name"] = None
    save_tickets(tickets)
...
async def close(ctx):
    ...
    cfg = get_config()
    if not has_any_role(ctx.author, [int(r) for r in cfg.get("ticket_support_roles", [])]):
        await ctx.send(embed=embed("❌ No Permission", "Only support roles can close.", color=0xED4245)); return
    tickets = get_tickets()
...
async def adduser(ctx, *, user_input: str):
    ...
    if not has_any_role(ctx.author, allowed):
        await ctx.send(embed=embed("❌ No Permission", "You don't have permission.", color=0xED4245)); return
    tickets = get_tickets()
    if str(ctx.channel.id) not in tickets:
        await ctx.send(embed=embed("❌ Error", "Use in a ticket channel.", color=0xED4245)); return
    member = None
    ...
    try:
        ...
        if not member:
            member = discord.utils.find(lambda m: search in m.name.lower() or search in m.display_name.lower(), ctx.guild.members)
    except discord.NotFound:
        pass
    if not member:
        await ctx.send(embed=embed("❌ Not Found", f"Could not find `{user_input}`.", color=0xED4245)); return
    await ctx.channel.set_permissions(member, read_messages=True, send_messages=True)
    await ctx.send(embed=embed("✅ User Added", f"{member.mention} added.", color=0x57F287))
...
async def cooked(ctx):
    ...
    cfg = get_config()
    if not has_any_role(ctx.author, [int(r) for r in cfg.get("ticket_support_roles", [])]):
        await ctx.send(embed=embed("❌ No Permission", "Only support roles.", color=0xED4245)); return
...
async def sendmsg(ctx, channel_id: str = None, *, message: str = None):
    ...
    if not channel_id or not message:
        await ctx.send(embed=embed("❌ Usage", "`!sendmsg <channel_id> <message>`", color=0xED4245)); return
    try:
        import re, io
        ch = ctx.guild.get_channel(int(channel_id))
        if not ch: await ctx.send(embed=embed("❌ Error", "Channel not found.", color=0xED4245)); return
...
async def setcookedrole(ctx, role_id: str):
    ...
    if not role: await ctx.send(embed=embed("❌ Error", "Role not found.", color=0xED4245)); return
    cfg["cooked_rich_role"] = role_id
    save_config(cfg)
    await ctx.send(embed=embed("✅ Updated", f"{role.mention} → 'I Wanna Be Rich' clickers.", color=0x57F287))
...
async def mminfoeng(ctx):
    ...
    info_embed.add_field(name="3️⃣ Proof of Payment", value="Buyer sends screenshot. Seller confirms.", inline=False)
...
""", inline=False)
    h.set_footer(text="Requires /activate <key> to unlock")
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.MissingPermissions):
        await ctx.send(embed=embed("❌ No Permission", "You need Administrator permission.", color=0xED4245))
        return
    print(f"Error: {error}")
